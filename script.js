...
function drawHeart(t){
  ...
  for(const p of heartPoints){
    ...
    ctx.fillStyle = `rgba(255,150,200,${a})`;
    ctx.beginPath();
...
/* load ảnh – CÙNG THƯ MỤC */
for(let i=1;i<=IMAGE_COUNT;i++){
  const img = new Image();
  img.src = `anh${i}.jpg`; // ⭐ QUAN TRỌNG
  images.push(img);
}
...
function drawBalloons(t){
  for(const b of balloons){
    ...
    /* viền bóng */
    ctx.strokeStyle = "rgba(255,200,220,.85)";
    ctx.lineWidth = 2;
    ctx.beginPath();
    ctx.arc(b.x, b.y, b.r, 0, Math.PI*2);
    ctx.stroke();
